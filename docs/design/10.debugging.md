...
| `src/decompile/stages.rs` | 阶段表同时绑定 run / dump 函数，按阶段执行 dump / timing，并把 pass dump 请求转交给 HIR/Readability |
...
1. 新增 stage dump：先在对应层实现渲染函数，再在该层 `debug.rs` 通过 `define_stage_dump!` 声明统一接收 `DecompileState + DebugOptions` 的入口，并在 `src/decompile/stages.rs` 的阶段表里绑定到对应 `DecompileStage`。
