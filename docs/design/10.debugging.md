...
CLI 参数
   ...
   -> pipeline 决定要不要 dump / timing / pass-dump
...
| `src/decompile/pipeline.rs` | 按阶段执行 dump / timing，并把 pass dump 请求转交给 HIR/Readability |
| `src/decompile/debug.rs` | stage dump 包装函数与 feature-gated 调用边界 |
...
2. `src/decompile/pipeline.rs` 把 pass 名列表传给 HIR / AST readability 入口。
3. `src/hir/analyze.rs` 组装 HIR `PassDumpConfig`；`src/ast/readability.rs` 直接消费 AST pass 名列表。
4. HIR / AST pass 在前后抓取快照，并只在实际变更时输出。
5. HIR 快照由 `src/hir/debug.rs` 渲染；AST readability 快照由 `src/ast/debug.rs` 渲染。

## 扩展调试设施时看这里

1. 新增 stage dump：先在对应层实现渲染函数，再接到 `src/decompile/debug.rs` 的 `define_stage_dump!` 包装函数，并由 pipeline 在对应阶段产物生成后调用。
2. 新增带 proto 概念的层：不要自写过滤规则，直接接入 `src/debug/focus.rs`。
3. 新增 pass-dump 能力：优先复用现有快照格式，不要另起一套输出协议。
4. 新增调试参数：入口放 `src/decompile/options.rs`，执行编排放 `src/decompile/pipeline.rs` 或对应业务层入口。
